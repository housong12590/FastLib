            ...
            if (path != null) {
                ...
                for (int i = 0; i < pathList.size(); i++) {
                    if (!FileUtils.isFile(pathList.get(i))) {
                        ToastUtils.show( "第" + (i + 1) + "张图片不存在，请检查后重试");
                        return;
                    }
